...
head. `deal_agent/findings.py` is the analysis register: thirteen findings, each
...
while NLPI funded 90% of the cheque (its own open question, asked twice), the worked
exits split the buyer's cheque on x1 where the structure says x2, and "LIQPREF
SATISFIED" is marked $0.49M — or $4.41M — early.

```bash
python -m unittest discover -s deal_agent/tests -t .          # 40 tests, no deps
```
