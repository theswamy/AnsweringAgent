...
the liquidation preference expressed as a formula rather than a multiple. Every figure in
it is checked against the model by `deal_agent/tests/test_uniform_rule.py`.
...
`docs/presentation/SB2-NLP-Secondary.pptx` is the same walkthrough as a PowerPoint deck,
for importing into Google Slides. It is generated by `docs/presentation/build-deck.js`
(`node build-deck.js`, then recompress — pptxgenjs writes the archive uncompressed):

```bash
cd docs/presentation && npm install pptxgenjs && node build-deck.js
```
